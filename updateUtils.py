...
class updateUtils:
    ...
    @staticmethod
    def extract_zip_to_directory(zip_file_path, output_dir):
        ...
        try:
            if os.path.exists(output_dir):
                try:
                    ...
                    msg = f'Faild to remove {output_dir} for zip extract: {e}'
                    return False, msg

            try:
                os.makedirs(output_dir)
            except Exception as e:
                msg = f'Faild to create {output_dir} for zip extract: {e}'       
                return False
            ...
            try:
                ...
                msg = f'Bad zip file {zip_file_path}'
                return False, msg
            except FileNotFoundError:
                msg = f'zip file not exist {zip_file_path}'
                return False, msg
            except Exception as e:
                msg = f'unexcept error happend in extract zip {zip_file_path}: {e}'
                return False, msg

        except Exception as e:
            msg = f'some error happend in extract zip {zip_file_path}: {e}'
            return False, msg
    ...
    def remove_specific_metadata(file_path, key):
        ...
        if separator in data:
            ...
            metadata_str:str = metadata.decode('utf-8')
    ...
    #---------------------------------------------------------------------------------------------------------------
    @staticmethod
    def encrypt_zip_file(zip_file_path, password:str, out_path):
    ...
    #---------------------------------------------------------------------------------------------------------------
    @staticmethod
    def decrypt_zip_file(encrypted_zip_path, password:str, output_zip_path):
    ...
    def get_metadata_value(file_path, key):
        # خواندن متا از فایل
        metadata = updateUtils.get_metadata_from_file(file_path)
        if metadata:
